...
export const usePassbookPrint = () => {
  ...
  const { printType, memberNo } = form.watch();
  ...
  const getPassbokFrontPageApiCall = async (item) => {
    ...
    try {
      const res = await getPassbookPrintAPI(orgId, item.memberNo, "", "", 1);
  ...
  const getPassbokTransParamsApiCall = async (memberNo) => {
    setLoading(true);

    try {
      const res = await getPassbookPrintAPI(orgId, memberNo, "", "", 2);
  ...
  useEffect(() => {
    if (printType === "2" && memberNo) {
      getPassbokTransParamsApiCall(memberNo);
    }
  }, [printType, memberNo]);
