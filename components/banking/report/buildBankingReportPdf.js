...
const columnsFor = (t) => [
  { title: label(t, "bank.print.slNo", "SL. NO."), w: 14, align: "center", key: "sl" },
  { title: label(t, "bank.print.bankName", "BANK NAME"), w: 46, align: "left", key: "name" },
  { title: label(t, "bank.print.accountNo", "ACCOUNT NO."), w: 28, align: "center", key: "acc" },
  { title: label(t, "bank.print.accountType", "ACCOUNT TYPE"), w: 26, align: "center", key: "type" },
  { title: label(t, "bank.print.opening", "OPENING"), w: 21, align: "right", key: "opening" },
  { title: label(t, "bank.print.deposit", "DEPOSIT"), w: 21, align: "right", key: "deposit" },
  { title: label(t, "bank.print.withdrawn", "WITHDRAWN"), w: 21, align: "right", key: "withdrawn" },
  { title: label(t, "bank.print.closing", "CLOSING"), w: 21, align: "right", key: "closing" },
];
...
const flattenGroups = (groups = [], t) => {
    ...
    (group?.transactions || []).forEach((txn, index) => {
      rows.push({
        ...
        cells: {
          ...
          acc: txn?.Account_No || "",
          type: txn?.Acct_Type || "",
          opening: money(txn?.Opening),
          deposit: money(txn?.Deposit),
          withdrawn: money(txn?.Withdrawn ?? txn?.Withdrwan),
          closing: money(txn?.Closing),
        },
      });
    });
    rows.push({
      kind: "total",
      label: label(t, "bank.subTotal", "Sub Total"),
      opening: money(group?.subtotalOpening),
      deposit: money(group?.subtotalDeposit),
      withdrawn: money(group?.subtotalWithdrawn),
      closing: money(group?.subtotalClosing),
    });
  });
  ...
  if (grand) {
    rows.push({
      kind: "total",
      label: label(t, "bank.grandTotal", "Grand Total"),
      opening: money(grand.grandTotalOpening),
      deposit: money(grand.grandTotalDeposit),
      withdrawn: money(grand.grandTotalWithdrawn),
      closing: money(grand.grandTotalClosing ?? grand.grandTotalClose),
    });
  }
  return rows;
};
...
export const downloadBankingReportPdf = async ({
  ...
  const range = `${day(fromDate)} ${label(t, "bank.to", "To")} ${day(toDate)}`;
  const meta = {
    ...
    title: `${label(t, "bank.detailedListFrom", "Bank Detailed List From")} ${range}`,
    generatedBy: label(t, "bank.generatedBy", "Generated By :"),
    generatedOn: label(t, "bank.generatedOn", "Generated On :"),
    generatedNote: label(
      t,
      "bank.reportGeneratedByPrioSuite",
    ...
    if (row.kind === "banner") {
      ...
      paintCells(
        columns.map((col) => {
          ...
          if (col.key === "opening") return row.opening;
          if (col.key === "deposit") return row.deposit;
          if (col.key === "withdrawn") return row.withdrawn;
          if (col.key === "closing") return row.closing;
          return "";
        }),
        true,
      );
