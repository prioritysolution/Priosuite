    ...
    },
    {
      ...
      header: ({ column }) => {
        return (
          ...
          >
            {t("master.operationalArea.table.policeStationName")}
      ...
      cell: ({ row }) => {
        return (
          <div className="text-left px-5">{row.getValue("STation_Name")}</div>
        );
      },
    },
    {
          ...
          <PaginationContent>
            <PaginationItem>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setCurrentPoliceStationPage((prev) => prev - 1)}
                disabled={currentPoliceStationPage === 1}
              >
                {t("common.previous")}
              </Button>
            </PaginationItem>

            {Array.from({ length: lastPoliceStationPage }).map((_, i) => (
              <PaginationItem
                key={i}
                onClick={() => setCurrentPoliceStationPage(i + 1)}
              >
            ...
            <PaginationItem>
              <Button
                ...
                size="sm"
                onClick={() => setCurrentPoliceStationPage((prev) => prev + 1)}
                disabled={currentPoliceStationPage === lastPoliceStationPage}
              >
                {t("common.next")}
              </Button>
            </PaginationItem>
