...
import { downloadBorrowingsReportPdf } from "./buildBorrowingsReportPdf";
...
const BorrowingsReport = ({
  ...
  const generateDetailedListPrint = useReactToPrint({
    ...
    documentTitle: `BorrowingsDetailedList-${
  ...
  const handleDownloadPDF = async () => {
    ...
    if (!(tableData?.length > 0)) {
      toast.error("No borrowings report data to download.");
      return;
    }

    const docTitle = `BorrowingsDetailedList-${formatReportDate(fromDate)}-${formatReportDate(toDate)}`;
    const toastId = toast.loading("Preparing PDF…");

    try {
      setPdfLoading(true);
      await downloadBorrowingsReportPdf({
        showData,
        tableData,
        fromDate,
        toDate,
            ...
            <div className="w-full flex flex-col border border-primary rounded-lg overflow-hidden">
              ...
              >
                ...
                <h3 className="text-xl font-semibold ">
                  {tEn("borrowings.borrowingsReport")}
                </h3>
                <div
